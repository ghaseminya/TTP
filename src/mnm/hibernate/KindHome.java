...
/**
 * Home object for domain model class Kind.
 * @see .Kind
 * @author Hibernate Tools
 */
public class KindHome {

	private static final Log log = LogFactory.getLog(KindHome.class);
	...
	public void persist(Kind transientInstance) {
		log.debug("persisting Kind instance");
		try {
	...
	public Kind merge(Kind detachedInstance) {
		...
		try {
			Kind result = (Kind) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Kind findById(java.lang.Integer id) {
		...
		try {
			Kind instance = (Kind) sessionFactory.getCurrentSession().get(
					"Kind", id);
			if (instance == null) {
	...
	public List<Kind> findByExample(Kind instance) {
		log.debug("finding Kind instance by example");
		try {
			List<Kind> results = (List<Kind>) sessionFactory
					.getCurrentSession().createCriteria("Kind").add(
